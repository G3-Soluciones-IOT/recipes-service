    ...
    public SecurityFilterChain filterChain(
        ...
        http.authorizeHttpRequests(authorize -> authorize
                ...
                .requestMatchers(HttpMethod.GET, "/**").access((authentication, context) ->
                        hasPermissionOrLegacyJwt((Supplier<Authentication>) authentication, context, legacyJwtIssuer, "read:recipes"))
                .requestMatchers(HttpMethod.HEAD, "/**").access((authentication, context) ->
                        hasPermissionOrLegacyJwt((Supplier<Authentication>) authentication, context, legacyJwtIssuer, "read:recipes"))
                .anyRequest().access((authentication, context) ->
                        hasPermissionOrLegacyJwt((Supplier<Authentication>) authentication, context, legacyJwtIssuer, "write:recipes")));

        http.oauth2ResourceServer(oauth2 -> oauth2.jwt(jwt ->
                jwt.jwtAuthenticationConverter(RecipesJwtAuthenticationConverter.jwtAuthenticationConverter())));
